...
impl ImageRGBA<u8> {
    ...
    pub fn filled(width: usize, height: usize, value: [u8; 4]) -> Self {
        debug_assert!(width > 0 && height > 0);
        let mut data = Array3::zeros((height, width, 4));
    ...
    pub fn from_layers(layers: [Array2<u8>; 4]) -> Self {
    ...
    pub fn get_pixel(&self, coords: [usize; 2]) -> [u8; 4] {
    ...
    pub fn set_pixel(&mut self, coords: [usize; 2], pixel: [u8; 4]) {
    ...
    pub fn save<P: AsRef<Path>>(&self, path: P) -> Result<(), ImageError> {
        ...
        // Flip the image vertically for proper orientation.
    ...
    /// Loads an RGBA PNG image from the specified path.
...
impl std::fmt::Display for ImageRGBA<u8> {

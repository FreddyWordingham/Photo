...
impl<T: NormFloat> ImageRGBA<T> {
    ...
    /// Loads an RGBA PNG image and converts it to a normalized ImageRGBA.
...
impl<T: NormFloat> std::fmt::Display for ImageRGBA<T> {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        for row in self.data.outer_iter().rev() {
            for pixel in row.outer_iter() {
                ...
                let a = pixel[3].max(T::zero()).min(T::one()) * T::from(255).unwrap();
                ...
                let a_u8 = <u8 as NumCast>::from(a.round()).unwrap();
                write!(f, "\x1b[48;2;{r_u8};{g_u8};{b_u8};{a_u8}m  \x1b[0m")?;
            }
            writeln!(f)?;
        }
        Ok(())

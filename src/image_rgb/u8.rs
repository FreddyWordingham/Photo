...
impl ImageRGB<u8> {
    ...
    pub fn filled(width: usize, height: usize, value: [u8; 3]) -> Self {
        debug_assert!(width > 0);
        debug_assert!(height > 0);
    ...
    /// Creates an ImageRGB from three colour layers.
    pub fn from_layers(layers: [Array2<u8>; 3]) -> Self {
    ...
    pub fn get_pixel(&self, coords: [usize; 2]) -> [u8; 3] {
    ...
    pub fn set_pixel(&mut self, coords: [usize; 2], pixel: [u8; 3]) {
        let mut view = self.data.slice_mut(s![coords[1], coords[0], ..]);
        view.assign(&arr1(&pixel));
    }

    /// Get a colour channel of the image.
...
impl std::fmt::Display for ImageRGB<u8> {

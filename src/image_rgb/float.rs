...
impl ImageRGB<f32> {
    ...
    pub fn filled(width: usize, height: usize, value: [f32; 3]) -> Self {
        debug_assert!(width > 0);
        debug_assert!(height > 0);
    ...
    pub fn from_layers(layers: [Array2<f32>; 3]) -> Self {
    ...
    pub fn get_pixel(&self, coords: [usize; 2]) -> [f32; 3] {
    ...
    pub fn set_pixel(&mut self, coords: [usize; 2], pixel: [f32; 3]) {
    ...
    /// Values are clamped to [0.0, 1.0] and converted to u8.
    ...
    /// Loads an RGB PNG image and converts it to float representation.
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> Result<Self, ImageError> {
        ...
        let image_array = ndarray::Array3::from_shape_vec((height, width, channels), data_vec)
            .map_err(|err| {
                ImageError::from_message(format!("Failed to create image array: {}", err))
            })?;

        // Flip vertically and convert u8 to f32 (normalized).
...
impl std::fmt::Display for ImageRGB<f32> {
